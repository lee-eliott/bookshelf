import { enhanceCoverUrl, stripHtml } from "./utils";
  ...
  try {
    ...
    if ((!cover_url || !description) && volumeInfo.title) {
      try {
        ...
        const enrichRes = await fetchGoogleBooks(
          "volumes",
          { q: parts.join(" "), maxResults: 5 },
          86400
        );
        if (enrichRes.ok) {
...
/**
 * Resolves a book from its ISBN across Google Books, Open Library and the BnF.
 * Sources are queried in parallel; each field takes the first non-null value
 * in GB > OL > BnF order, so one source being down degrades data rather than
 * failing the lookup.
 */
export async function resolveBookByIsbn(isbn: string): Promise<BookResult | null> {
  ...
  if (!gbBook && !olBook && !bnfBook) return null;

  // Cover: GB → OL data API → OL CDN HEAD → BnF (already HEAD-checked in fromBnF)
  const cover_url =
    gbBook?.cover_url ??
    olBook?.cover_url ??
  ...
  return {
    isbn,
    title: gbBook?.title ?? olBook?.title ?? bnfBook?.title ?? "Titre inconnu",
    author: gbBook?.author ?? olBook?.author ?? bnfBook?.author ?? null,
    cover_url,
    description: stripHtml(
      gbBook?.description ?? olBook?.description ?? bnfBook?.description
    ),
    published_year:
      gbBook?.published_year ??
      olBook?.published_year ??
      bnfBook?.published_year ??
      null,
  };
}
